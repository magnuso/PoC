...
BEGIN
  ...
  bodyOfProc: BEGIN
    ...
    IF inReceiptId > 0 AND pocPocPathFromId(inReceiptId) IS NULL THEN
      SELECT 404 AS id, 'Not Found' AS name, 'pocAttributeTextInsert receiptId' AS content;
      LEAVE bodyOfProc;
    END IF;
    -- finally
    SET t = UNIX_TIMESTAMP();
    INSERT INTO pocAttributeText (creditId, debitId, receiptId, created, createdById, modified, modifiedById, className, name, title, content, value)
