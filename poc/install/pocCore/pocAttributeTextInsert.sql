...
CREATE PROCEDURE pocAttributeTextInsert (
...
BEGIN
  ...
  bodyOfProc: BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION SELECT 400 AS id, 'SQLEXCEPTION' AS name, 'pocAttributeTextInsert' AS content;
    -- check
    IF pocPocPathFromId(inCreditId) IS NULL THEN
      SELECT 404 AS id, 'Not Found' AS name, 'pocAttributeTextInsert' AS content;
      LEAVE bodyOfProc;
    END IF;
    IF NOT pocPocCheckPriv(inCreditId, 'insert') THEN
      SELECT 401 AS id, 'Unauthorized' AS name, 'pocAttributeTextInsert' AS content;
      LEAVE bodyOfProc;
    END IF;
    IF inDebitId > 0 AND pocPocPathFromId(inDebitId) IS NULL THEN
      SELECT 404 AS id, 'Not Found' AS name, 'pocAttributeTextInsert debitId' AS content;
      LEAVE bodyOfProc;
    END IF;
    IF inVoucherId > 0 AND pocPocPathFromId(inVoucherId) IS NULL THEN
      SELECT 404 AS id, 'Not Found' AS name, 'pocAttributeTextInsert voucherId' AS content;
      LEAVE bodyOfProc;
    END IF;
    -- finally
    SET t = UNIX_TIMESTAMP();
    INSERT INTO pocAttributeText (creditId, debitId, voucherId, created, createdById, modified, modifiedById, className, name, title, content, value)
