...
BEGIN
  ...
  bodyOfProc: BEGIN
    ...
    IF inVoucherId > 0 AND pocPocPathFromId(inVoucherId) IS NULL THEN
      SELECT 404 AS id, 'Not Found' AS name, 'pocAttributeDoubleInsert voucherId' AS content;
      LEAVE bodyOfProc;
    END IF;
    -- finally
    SET t = UNIX_TIMESTAMP();
    INSERT INTO pocAttributeDouble (creditId, debitId, voucherId, created, createdById, modified, modifiedById, className, name, title, content, value)
