...
CREATE PROCEDURE pocAttributeCharInsert (
...
BEGIN
  ...
  bodyOfProc: BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION SELECT 400 AS id, 'SQLEXCEPTION' AS name, 'pocAttributeCharInsert' AS content;
    -- check
    IF pocPocPathFromId(inCreditId) IS NULL THEN
      SELECT 404 AS id, 'Not Found' AS name, 'pocAttributeCharInsert' AS content;
      LEAVE bodyOfProc;
    END IF;
    IF NOT pocPocCheckPriv(inCreditId, 'insert') THEN
      SELECT 401 AS id, 'Unauthorized' AS name, 'pocAttributeCharInsert' AS content;
      LEAVE bodyOfProc;
    END IF;
