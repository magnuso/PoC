...
BEGIN
  ...
  bodyOfProc: BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION SELECT 400 AS id, 'SQLEXCEPTION' AS name, 'pocAttributeIntInsert' AS content;
    -- check
    ...
    IF inReceiptId > 0 AND pocPocPathFromId(inReceiptId) IS NULL THEN
      SELECT 404 AS id, 'Not Found' AS name, 'pocAttributeIntInsert receiptId' AS content;
      LEAVE bodyOfProc;
    END IF;
    -- finally
    SET t = UNIX_TIMESTAMP();
    INSERT INTO pocAttributeInt (creditId, debitId, receiptId, created, createdById, modified, modifiedById, className, name, title, content, value)
