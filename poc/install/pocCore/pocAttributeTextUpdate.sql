...
BEGIN
  ...
  bodyOfProc: BEGIN
    ...
    IF pocPocPathFromId(inCreditId) IS NULL THEN
      SELECT 404 AS id, 'Not Found' AS name, 'pocAttributeTextUpdate' AS content;
      LEAVE bodyOfProc;
    END IF;
    IF NOT pocPocCheckPriv(inCreditId, 'update') THEN
      SELECT 401 AS id, 'Unauthorized' AS name, 'pocAttributeTextUpdate' AS content;
      LEAVE bodyOfProc;
    END IF;
    ...
    UPDATE pocAttributeText SET creditId = inCreditId, debitId = inDebitId, voucherId = inVoucherId, 
