...
BEGIN
  ...
  bodyOfProc: BEGIN
    ...
    IF inVoucherId > 0 AND pocPocPathFromId(inVoucherId) IS NULL THEN
      SELECT 404 AS id, 'Not Found' AS name, 'pocAttributeCharUpdate voucherId' AS content;
      LEAVE bodyOfProc;
    END IF;
    -- finally
    SET t = UNIX_TIMESTAMP();
    UPDATE pocAttributeChar SET creditId = inCreditId, debitId = inDebitId, voucherId = inVoucherId, 
