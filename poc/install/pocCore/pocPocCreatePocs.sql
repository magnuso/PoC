...
BEGIN
  ...
  bodyOfProc: BEGIN
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION SELECT 400 AS id, 'SQLEXCEPTION' AS name, 'pocPocCreatePocs' AS content;
    OPEN cur;
    thePocLoop: LOOP
      ...
      -- poc
      SELECT 'poc' AS className, loopPath AS path, tp.id, tp.parentId, tp.groupId, tp.userId, tp.groupId, tp.created, tp.createdById, tp.modified, tp.modifiedById,
        ...
        WHERE tp.id = loopId;
      -- pocAttributeChar
      SELECT ta.*, runPriv, openPriv, selectPriv, insertPriv, updatePriv, deletePriv,
          tuc.name AS createdByName, tum.name AS modifiedByName
